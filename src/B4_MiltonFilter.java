public class B4_MiltonFilter extends World {
   ...
   public void go() {
      plane.loadBackGround("mickey4.jpg");          //*** new method to load a diffrent JPG
      System.out.println("This will be printed to the  window. ");
      plane.teleport(0, 0);
      plane.showBackGround();
      System.out.println(plane.howMuchRed());

      for (int row = 350; row < 500; row++) {
         for (int col = 140; col < 500; col++) {
            ...
            if ((red >= 150 && green >= 0 && blue >= 0) && (green < 210 && blue < 230)) {
               plane.setPixelColor(255, 9, 200);
            }

         }
      }

      for (int row = 230; row < 270; row++) {
         for (int col = 210; col < 310; col++) {
            ...
            if (red <= 206 && green <= 156 && blue <= 255)
               plane.setPixelColor(251, 207, 168);
         }
      }

      for (int row = 203; row < 235; row++) {
         for (int col = 253; col < 309; col++) {
